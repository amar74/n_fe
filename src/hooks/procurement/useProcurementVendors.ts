...
export function useProcurementVendors() {
  ...
  const createVendorMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Supplier has been created successfully. Invitation email sent.');
    },
    onError: (error: any) => {
  ...
  const updateVendorMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: procurementVendorKeys.lists() });
      toast.success('Supplier details have been updated successfully.');
    },
    onError: (error: any) => {
  ...
  const updateVendorStatusMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: procurementVendorKeys.lists() });
      queryClient.invalidateQueries({ queryKey: procurementVendorKeys.stats() });
      toast.success('Supplier status has been updated successfully.');
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.detail || 'Failed to update vendor status');
    },
  });
