...
export function useAuth() {
  ...
  const signIn = useCallback(async (email: string, password: string) => {
    try {
      ...
      console.log('[useAuth] API Base URL:', API_BASE_URL_WITH_PREFIX);
      console.log('[useAuth] Full login URL will be:', `${API_BASE_URL_WITH_PREFIX}/auth/login`);
      ...
      if (err.response) {
        ...
        const status = err.request.status || 'N/A';
        const statusText = err.request.statusText || 'No response';
        const fullUrl = err.config?.baseURL && err.config?.url 
          ? `${err.config.baseURL}${err.config.url}`
          : err.config?.url || 'Unknown URL';
        
        errorMessage = `No response from server (${status}). Please check if the backend is running at ${API_BASE_URL_WITH_PREFIX} and try again.`;
        console.error('[useAuth] Login network error:', {
          request: err.request,
          status,
          statusText,
